...
const schema = z.object({
  categoryFa: z.string().max(120).optional(), categoryEn: z.string().max(120).optional(),
  platforms: z.string().max(240).optional(), version: z.string().max(120).optional(), documentationUrl: optionalUrl.optional(),
  ...
  audience: z.enum(["all", "student", "teacher", "admin", "member"]),
...
export async function saveResource(
  ...
  const kind = resourceKind.parse(kindValue);
  ...
  try {
    ...
    else await getDatabase().insert(resources).values(data);
  } catch {
    return failed(locale);
  }
  revalidatePath("/", "layout");
  redirect(`/panel/admin/resources/${kind}?saved=1`);
}
