...
import { deleteOfflineSession } from '@/lib/api/panel/teacher/offline_sessions';
...
export function CreateOfflineSession() {
  return (
    ...
    >
      <span className="hidden md:block">ساخت کلاس آفلاین</span>{' '}
      <PlusIcon className="h-5 md:mr-4" />
    </Link>
  );
}

export function UpdateOfflineSession({ id }: { id: string }) {
  return (
    <Link
      href={`/teacher/offline_sessions/${id}/edit`}
...
export function DeleteOfflineSession({ id }: { id: string }) {
  ...
  return (
    <>
      ...
      <Modal
        ...
        description="آیا از حذف این کلاس آفلاین مطمئن هستید؟"
        confirmText="حذف"
        cancelText="انصراف"
        loading={isDeleting}
      />
