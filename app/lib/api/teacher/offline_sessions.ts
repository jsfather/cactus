...
export const getOfflineSessions = async () => {
  const response = await request<{ data: OfflineSession[] }>(
    'teacher/offline_sessions'
  );
...
export const getOfflineSession = async (id: string) => {
  const response = await request<OfflineSession>(
    `teacher/offline_sessions/${id}`
  );
