...
import { CreateStudentRequest, UpdateStudentRequest } from '@/app/lib/types/student';
...
const StudentFormPage: React.FC<PageProps> = ({ params }) => {
  ...
  const { createStudent, updateStudent, fetchStudentById, currentStudent, clearCurrentStudent } = useStudent();
  ...
  const onSubmit = async (data: StudentFormData) => {
    try {
      ...
      if (isNew) {
        ...
        await updateStudent(resolvedParams!.id, formData as UpdateStudentRequest);
...
export default StudentFormPage;
      ...
      try {
        ...
        reset({
          ...
          level_id:
            typeof student.level_id === 'string'
              ? parseInt(student.level_id)
              : student.level_id || 1,
          ...
          has_allergy:
            typeof student.has_allergy === 'string'
              ? parseInt(student.has_allergy)
              : student.has_allergy || 0,
  ...
  const handleError = (error: ApiError) => {
    ...
    if (error?.message) {
      ...
      toast.error(
        isNew ? 'خطا در ایجاد دانش‌آموز' : 'خطا در بروزرسانی دانش‌آموز'
      );
    }
  };
