...
import { getTerm } from '@/app/lib/api/teacher/terms';
...
export default function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main>
      <Breadcrumbs
        breadcrumbs={[
          { label: 'ترم‌ها', href: '/teacher/terms' },
          {
            label: 'مشاهده ترم',
            href: `/teacher/terms/${resolvedParams.id}`,
            active: true,
          },
        ]}
      />
      ...
      <div className="mt-8 space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          <div>
            <h3 className="text-sm font-medium text-gray-500">ظرفیت</h3>
            <p className="mt-1 text-sm text-gray-900">{term.capacity}</p>
          </div>

          <div>
            <h3 className="text-sm font-medium text-gray-500">سطح</h3>
            <p className="mt-1 text-sm text-gray-900">{term.level_id}</p>
          </div>
        </div>
