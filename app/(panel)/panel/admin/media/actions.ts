    ...
    return {
      error:
        formData.get("locale") === "en"
          ? "Please review the media information."
          : "لطفاً اطلاعات رسانه را بررسی کنید.",
    ...
    return {
      error:
        parsed.data.locale === "en"
          ? "This media item no longer exists."
          : "این رسانه دیگر وجود ندارد.",
    };
  revalidateMedia();
...
export async function deleteMediaAsset(
  ...
  const [
    ...
    database
      ...
      .where(
        or(
          like(posts.coverImageUrl, pattern),
          like(posts.contentFa, pattern),
          like(posts.contentEn, pattern),
        ),
      )
      .limit(1),
    database
      ...
      .where(
        or(
          like(products.coverImageUrl, pattern),
          like(products.contentFa, pattern),
          like(products.contentEn, pattern),
        ),
      )
      .limit(1),
    database
      .select({ id: honors.id })
      .from(honors)
      .where(like(honors.certificateImageUrl, pattern))
      .limit(1),
    database
      ...
      .where(
        or(
          like(coursePages.coverImageUrl, pattern),
          like(coursePages.certificateImageUrl, pattern),
          like(coursePages.contentFa, pattern),
          like(coursePages.contentEn, pattern),
        ),
      )
      .limit(1),
    database
      ...
      .where(
        or(
          like(learningActivities.contentFa, pattern),
          like(learningActivities.contentEn, pattern),
        ),
      )
      .limit(1),
    database
      ...
      .where(
        or(
          like(resources.contentFa, pattern),
          like(resources.contentEn, pattern),
        ),
      )
      .limit(1),
  ]);
  ...
  if (
    ...
    return {
      error:
        locale === "en"
          ? "This image is currently used by a profile or published content. Remove it there first."
          : "این تصویر در پروفایل یا محتوای سایت استفاده شده است. ابتدا آن را از محل استفاده حذف کنید.",
    };
  }
    ...
    return {
      error:
        locale === "en"
          ...
          : "مسیر ذخیره‌شده رسانه معتبر نیست.",
    };
  }
  ...
  try {
    ...
    if (
      ...
      return {
        error:
          locale === "en"
            ? "The media file could not be removed from storage."
