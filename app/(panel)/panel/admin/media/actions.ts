...
import { mediaAssets, posts, products, users } from "@/lib/db/schema";
...
export async function deleteMediaAsset(assetIdValue: string, locale: Locale, redirectAfterDelete = false): Promise<DeleteMediaState> {
  ...
  const [avatarUse, postUse, productUse] = await Promise.all([
  ...
  if (avatarUse.length || postUse.length || productUse.length) {
