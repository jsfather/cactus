...
export default async function TeacherTermPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PanelPage><PanelBackLink href="/panel/teacher/terms">{isFa ? "بازگشت" : "Back"}</PanelBackLink><PanelPageHeader eyebrow={isFa ? "ترم اختصاص‌داده‌شده" : "Assigned term"} title={locale === "en" ? term.titleEn || term.titleFa : term.titleFa} description={isFa ? "برنامه، دانش پژوهان و ثبت‌نام این ترم را مدیریت کنید." : "Manage this term's schedule, students, and enrollment."} /><WeeklySchedule entries={entries} locale={locale} /><TermEnrollmentManager termId={term.id} locale={locale} students={students} roster={roster} invitations={invitations} enrollmentOpen={term.status === "enrollment_open"} /></PanelPage>;
}
