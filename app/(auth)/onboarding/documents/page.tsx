...
export default function OnboardingDocumentsPage() {
  ...
  const handleFileChange = (type: 'national_card' | 'certificate') => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push('/user'); // Redirect to user dashboard after completion
    } catch (error) {
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
          <div className="flex items-start">
            <FileText className="h-5 w-5 text-blue-500 mt-0.5 ml-3" />
            <div>
        ...
        <div className="flex gap-4">
          <button
            type="button"
            onClick={() => router.back()}
            className="flex-1 rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 focus:outline-none dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
          >
            بازگشت
          </button>
